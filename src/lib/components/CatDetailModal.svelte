<script lang="ts">
	...
	$effect(() => {
		currentIndex; // reactive dependency — re-init when cat changes
	...
	<div class="relative flex w-full items-center justify-center">
		...
		{#if prevCat}
			<button
				...
				class="absolute left-0 z-10 w-[22vw] max-w-[110px] cursor-pointer opacity-50 transition-opacity hover:opacity-70 sm:left-4"
			>
				...
				<!-- right-side shadow so it blends into center -->
		...
		{#if nextCat}
			<button
				...
				class="absolute right-0 z-10 w-[22vw] max-w-[110px] cursor-pointer opacity-50 transition-opacity hover:opacity-70 sm:right-4"
			>
				<div class="overflow-hidden rounded-2xl bg-white shadow-lg">
					<div class="aspect-square overflow-hidden bg-pink/30">
						{#if nextCat.avatar}
				...
				<!-- left-side shadow so it blends into center -->
